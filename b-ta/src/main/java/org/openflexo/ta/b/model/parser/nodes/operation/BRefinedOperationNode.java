 ...
 * 
 */
public class BRefinedOperationNode extends BOperationNode<ARefinedOperation, BRefinedOperation> {

	public BRefinedOperationNode(ARefinedOperation astNode, BSemanticsAnalyzer analyser) {
		super(astNode, analyser);
	}

	public BRefinedOperationNode(BRefinedOperation concept, BSemanticsAnalyzer analyser) {
		super(concept, analyser);
	}

	@Override
	public BRefinedOperation buildModelObjectFromAST(ARefinedOperation astNode) {
		getAnalyser().push(this);
		BRefinedOperation returned = getModelFactory().makeOperation(BRefinedOperation.class, getAnalyser().getBComponent());
		returned.setName(getText(astNode.getOpName()));
		returned.setRefinementKeyword(astNode.getRefKw().getText());
		returned.setRefinedOperationName(astNode.getAbOpName().getText());
	...
	protected RawSourceFragment getRefinedKeywordFragment() {
		if (getASTNode() != null) {
			return getFragment(getASTNode().getRefKw());
		}
		return null;
	}

	protected RawSourceFragment getRefinedOperationNameFragment() {
		if (getASTNode() != null) {
			return getFragment(getASTNode().getAbOpName());
		}
		return null;
	}
