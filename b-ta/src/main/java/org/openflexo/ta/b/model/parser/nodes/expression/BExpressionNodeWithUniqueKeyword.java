 ...
 * 
 */
public abstract class BExpressionNodeWithUniqueKeyword<N extends Node, T extends BExpression> extends BExpressionNode<N, T> {

	public BExpressionNodeWithUniqueKeyword(N astNode, BSemanticsAnalyzer analyser) {
		super(astNode, analyser);
	}

	public BExpressionNodeWithUniqueKeyword(T concept, BSemanticsAnalyzer analyser) {
		super(concept, analyser);
	}

	@Override
	public T buildModelObjectFromAST(N astNode) {
		getAnalyser().push(this);
		T returned = getModelFactory().makeExpression(getExpressionClass(), getAnalyser().getBComponent());
		getAnalyser().pop();
		return returned;
	}
