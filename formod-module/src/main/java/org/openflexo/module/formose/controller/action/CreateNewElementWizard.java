...
public class CreateNewElementWizard extends FlexoActionWizard<CreateNewElement> {

	@SuppressWarnings("unused")
	private static final Logger logger = Logger.getLogger(CreateNewElementWizard.class.getPackage().getName());

	private final ConfigureElement configuration;

	private static final Dimension DIMENSIONS = new Dimension(600, 400);

	public CreateNewElementWizard(CreateNewElement action, FlexoController controller) {
		super(action, controller);
		addStep(configuration = new ConfigureElement());
	}
	...
	public String getWizardTitle() {
		return getAction().getLocales().localizedForKey("create_children_element");
	}
	...
	@FIBPanel("Fib/Wizard/ConfigureElement.fib")
	public class ConfigureElement extends WizardStep {

		public ApplicationContext getServiceManager() {
			return getController().getApplicationContext();
		}

		public CreateNewElement getAction() {
			return CreateNewElementWizard.this.getAction();
		}

		@Override
		public String getTitle() {
			return getAction().getLocales().localizedForKey("configure_new_element");
		}
