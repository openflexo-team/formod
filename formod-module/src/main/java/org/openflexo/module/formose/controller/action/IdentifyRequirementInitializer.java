 ...
 * @author sylvain
 */
public class IdentifyRequirementInitializer extends ActionInitializer<IdentifyRequirement, FormoseProjectNature, FlexoObject> {

	public IdentifyRequirementInitializer(final ControllerActionInitializer controllerActionInitializer) {
		super(IdentifyRequirement.ACTION_TYPE, controllerActionInitializer);
	}

	@Override
	protected FlexoActionRunnable<IdentifyRequirement, FormoseProjectNature, FlexoObject> getDefaultInitializer() {
		return (e, action) -> {
			if (getController().getCurrentModuleView() instanceof DocXRequirementModuleView) {
				...
				action.setElement(moduleView.getPerspective().getReferencesBrowser().getFIBController().getFocusedElement());
			}

			Wizard wizard = new IdentifyRequirementWizard(action, getController());
	...
	@Override
	protected FlexoActionRunnable<IdentifyRequirement, FormoseProjectNature, FlexoObject> getDefaultFinalizer() {
		return (e, action) -> {
	...
	protected Icon getEnabledIcon(FlexoActionFactory actionType) {
		return IconFactory.getImageIcon(FMSIconLibrary.REQUIREMENT_ICON, IconLibrary.NEW_MARKER);
	}
